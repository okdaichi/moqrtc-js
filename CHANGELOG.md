...
- `VideoAnalyserNode`: report `frameIndex` as the cumulative *input* frame count (it previously counted only analyzed frames, so with `analysisInterval > 1` it understated the true frame number); apply only the configurable outer EWMA to `activityLevel` instead of double-smoothing against inconsistent state; validate `analysisSize` / `historySize` / `analysisInterval` (0 or non-integer values previously produced `NaN` metrics or silently disabled analysis); and reuse a histogram buffer instead of allocating one per analyzed frame.
