	...
	try {
		if (
			...
			__privateSet(
				this,
				_ctx,
				__privateGet(this, _canvas).getContext("2d", { willReadFrequently: true }),
			);
		}
...
var VideoContext = class {
	...
	get currentTime() {
		if (__privateGet(this, _state) === "running") {
			return performance.now() / 1e3 - __privateGet(this, _startTime) -
				__privateGet(this, _pausedTime);
		}
	...
	resume() {
		...
		if (__privateGet(this, _state) === "suspended") {
			__privateSet(
				...
				_pausedTime,
				__privateGet(this, _pausedTime) +
					(performance.now() / 1e3 - __privateGet(this, _currentTime) -
...
var VideoEncodeNode = class extends VideoNode {
	...
	process(input) {
		...
		const clonedFrame = input.clone();
		try {
			__privateGet(this, _encoder).encode(clonedFrame, {
				keyFrame: __privateGet(this, _isKey).call(this),
			});
		} catch (e) {
