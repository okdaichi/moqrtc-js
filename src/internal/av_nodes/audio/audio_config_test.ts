...
Deno.test("audioEncoderConfig: returns supported config for valid options", async () => {
	...
	try {
		...
		const calledConfig =
			(mockAudioEncoder.isConfigSupported as any).calls[0].args[0];
...
Deno.test("audioEncoderConfig: uses default bitrate when not provided", async () => {
	...
	try {
		...
		assertEquals(mockAudioEncoder.isConfigSupported.calls.length, 1);
		const calledConfig =
			mockAudioEncoder.isConfigSupported.calls[0].args[0];
...
Deno.test("audioEncoderConfig: uses custom preferredCodecs when provided", async () => {
	...
	try {
		...
		assertEquals(
			mockAudioEncoder.isConfigSupported.calls[0].args[0].codec,
			"pcmu",
		);
		assertEquals(
			mockAudioEncoder.isConfigSupported.calls[1].args[0].codec,
			"opus",
		);
	} finally {
		restoreMocks();
