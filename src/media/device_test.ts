...
import { MediaDeviceContext, Device } from "./device.ts";
...
Deno.test("abstract Device", async (t) => {
        ...
        await t.step("initializes correctly", () => {
                using _fakeMap = setupFakeMediaDevices(devices);
                const context = new MediaDeviceContext();
                const device = new Device(context, "video", { enabled: true, preferred: "cam-1", constraints: { width: 1920 }, getDefaultDeviceId });
        ...
        await t.step("stop() clears stream", async () => {
                ...
                device.stream = (await navigator.mediaDevices.getUserMedia({video: true})).getVideoTracks()[0];
