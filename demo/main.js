			...
			new AudioEncoder({
				output: async (chunk, meta) => {
					await Promise.allSettled(
						Array.from(__privateGet(this, _dests), async ([dest, cancel]) => {
...
var _analysisSize,
	...
	_pendingHeight,
	_pendingTimestamp,
	_pendingPresentationTime,
	_idleCallbackId,
	_scheduleAnalysis,
	scheduleAnalysis_fn,
	_runDeferredAnalysis,
	runDeferredAnalysis_fn,
	_convertToGrayscale,
	convertToGrayscale_fn,
	_calculateIntraFrame,
	calculateIntraFrame_fn,
	_calculateInterFrame,
	calculateInterFrame_fn,
	_calculateDensity,
	calculateDensity_fn;
var VideoAnalyserNode = class extends VideoNode {
	constructor(context, options) {
		...
		const bufferSize = __privateGet(this, _analysisSize).width *
			__privateGet(this, _analysisSize).height;
	...
	getAnalysisHistory(array, metric) {
		...
		for (let i = 0; i < length; i++) {
			const idx = (__privateGet(this, _historyWriteIndex) - length + i +
				__privateGet(this, _historySize)) % __privateGet(this, _historySize);
...
scheduleAnalysis_fn = function (frame) {
	...
	try {
		if (
			!__privateGet(this, _canvas) || __privateGet(this, _canvas).width !== sampleWidth ||
			__privateGet(this, _canvas).height !== sampleHeight
		) {
			__privateSet(this, _canvas, new OffscreenCanvas(sampleWidth, sampleHeight));
			__privateSet(
				this,
				_ctx,
				__privateGet(this, _canvas).getContext("2d", {
...
runDeferredAnalysis_fn = function () {
	...
	__privateMethod(this, _convertToGrayscale, convertToGrayscale_fn).call(
		this,
		pixelData,
		__privateGet(this, _grayscaleBuffer),
		width,
		height,
	);
	const intraFrame = __privateGet(this, _enabledFeatures).intraFrame
		? __privateMethod(this, _calculateIntraFrame, calculateIntraFrame_fn).call(
			this,
			pixelData,
			__privateGet(this, _grayscaleBuffer),
			width,
	...
	const interFrame = __privateGet(this, _enabledFeatures).interFrame
		? __privateMethod(this, _calculateInterFrame, calculateInterFrame_fn).call(
			this,
			pixelData,
			__privateGet(this, _previousFrameBuffer),
			width,
			height,
		)
		: { frameDelta: 0, motionEnergy: 0, activityLevel: 0 };
	const density = __privateGet(this, _enabledFeatures).density
		? __privateMethod(this, _calculateDensity, calculateDensity_fn).call(
			this,
			__privateGet(this, _grayscaleBuffer),
			width,
			height,
	...
	if (__privateGet(this, _currentAnalysis) && __privateGet(this, _smoothingTimeConstant) > 0) {
		...
		interFrame.activityLevel = k * __privateGet(this, _currentAnalysis).activityLevel +
	...
	__privateSet(
		this,
		_historyWriteIndex,
		(__privateGet(this, _historyWriteIndex) + 1) % __privateGet(this, _historySize),
	);
	if (this.onanalysis) {
		try {
	...
	if (
		__privateGet(this, _previousFrameBuffer) && __privateGet(this, _enabledFeatures).interFrame
	) {
...
calculateInterFrame_fn = function (currentPixels, previousPixels, width, height) {
	...
	const activityLevel = alpha * motionEnergy +
		(1 - alpha) * __privateGet(this, _previousMotionEnergy);
	...
	for (let y = 1; y < height - 1; y++) {
		for (let x = 1; x < width - 1; x++) {
			const gx = -1 * grayscale[(y - 1) * width + (x - 1)] +
				1 * grayscale[(y - 1) * width + (x + 1)] + -2 * grayscale[y * width + (x - 1)] +
				2 * grayscale[y * width + (x + 1)] + -1 * grayscale[(y + 1) * width + (x - 1)] +
				1 * grayscale[(y + 1) * width + (x + 1)];
			const gy = -1 * grayscale[(y - 1) * width + (x - 1)] -
				2 * grayscale[(y - 1) * width + x] - 1 * grayscale[(y - 1) * width + (x + 1)] +
				1 * grayscale[(y + 1) * width + (x - 1)] + 2 * grayscale[(y + 1) * width + x] +
				1 * grayscale[(y + 1) * width + (x + 1)];
...
var VideoDestinationNode = class extends VideoNode {
	constructor(context, canvas, options) {
		...
		__privateSet(
			this,
			_renderFunction,
			options?.renderFunction ?? VideoRenderFunctions.contain,
		);
	}
	get renderFunction() {
	...
	process(input) {
		...
		__privateSet(
			...
			requestAnimationFrame(() => {
				...
				void __privateMethod(this, _renderVideoFrame, renderVideoFrame_fn).call(
					this,
					frame,
				);
			}),
...
renderVideoFrame_fn = async function (frame) {
	...
	const { x, y, width, height } = __privateGet(this, _renderFunction).call(
		this,
		frame.displayWidth,
		frame.displayHeight,
		this.canvas.width,
...
var VideoContext = class {
	...
	async _waitNextFrame() {
		...
		if (__privateGet(this, _nextFrameTime) === 0) {
			...
			console.log(
				`[VideoContext] Initializing nextFrameTime to ${
					__privateGet(this, _nextFrameTime)
				}`,
			);
		}
		...
		console.log(
			`[VideoContext] delay=${delay}ms, nextFrameTime=${__privateGet(this, _nextFrameTime)}`,
		);
		if (delay > 0) {
...
var VideoEncodeNode = class extends VideoNode {
	constructor(context, options) {
		...
		__privateSet(
			...
			new VideoEncoder({
				output: (chunk, meta) => {
					console.log(
						`[VideoEncodeNode] output called: timestamp=${chunk.timestamp}, type=${chunk.type}, byteLength=${chunk.byteLength}, dests=${
							__privateGet(this, _dests2).size
						}`,
					);
					Promise.allSettled(
						Array.from(__privateGet(this, _dests2), async ([dest, cancel]) => {
	...
	process(input) {
		console.log(
			`[VideoEncodeNode] process called: timestamp=${input.timestamp}, disposed=${this.disposed}, state=${
				__privateGet(this, _encoder2).state
			}`,
		);
		...
		const clonedFrame = input.clone();
		try {
			const isKey = __privateGet(this, _isKey).call(
				this,
				input.timestamp,
				this.encodeQueueSize,
			);
			console.log(
				`[VideoEncodeNode] Encoding frame: keyFrame=${isKey}, queueSize=${this.encodeQueueSize}`,
			);
...
var VideoOverlayNode = class extends VideoNode {
	...
	process(input) {
		...
		try {
			...
			__privateGet(this, _overlay).call(
				this,
				__privateGet(this, _ctx2),
				__privateGet(this, _canvas2),
			);
		...
		const done = (async () => {
			...
			try {
				...
				while (__privateGet(this, _running) && this.context.state === "running") {
					console.log(
						`[VideoSourceNode] Loop iteration ${++frameCount}, running=${
							__privateGet(this, _running)
					...
					if (!__privateGet(this, _running) || !__privateGet(this, _reader)) {
						console.log(
							`[VideoSourceNode] Breaking: running=${
								__privateGet(this, _running)
							}, hasReader=${!!__privateGet(this, _reader)}`,
						);
						break;
					}
					...
					console.log(
						`[VideoSourceNode] Read result: done=${done2}, hasFrame=${!!frame}`,
					);
					console.log(
						`[VideoSourceNode] Read result: done=${done2}, hasFrame=${!!frame}`,
					);
